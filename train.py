...
def evaluate(args, model, data_loader, device):
    ...
    ## Write to folder for post procesing
    if 'test' in args.split:
        folder_path = args.save_dir
        with open(os.path.join(folder_path, 'X_batch.pk'), 'wb') as f:
            pickle.dump(X_batch, f)
        with open(os.path.join(folder_path, 'logits_batch.pk'), 'wb') as f:
            pickle.dump(logits_batch, f)
        with open(os.path.join(folder_path, 'gold_sums.pk'), 'wb') as f:
            pickle.dump(gold_summaries, f)
    results = {}
    pred = []
